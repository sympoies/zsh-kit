...

{
  ...
  output="$(CODEX_PROMPT_SEGMENT_CODEX_CLI="$stub" CODEX_STUB_LOG="$log" "$WRAPPER" --time-format=%Y 2>&1)"
  rc=$?
  ...
  assert_eq $'prompt-segment --time-format=%Y\nescape=true' "$logged" "render dispatch argv/env" || fail "$logged"

  : >| "$log"
  output="$(CODEX_PROMPT_SEGMENT_CODEX_CLI="$stub" CODEX_STUB_LOG="$log" "$WRAPPER" --is-enabled 2>&1)"
  rc=$?
  logged="$(command cat -- "$log")"
  ...
  assert_eq $'prompt-segment check\nescape=' "$logged" "--is-enabled should not request render escaping" || fail "$logged"

  : >| "$log"
  output="$(CODEX_PROMPT_SEGMENT_CODEX_CLI="$stub" CODEX_STUB_LOG="$log" "$WRAPPER" --status --format json 2>&1)"
  rc=$?
  logged="$(command cat -- "$log")"
  ...
  assert_eq $'prompt-segment status --format json\nescape=' "$logged" "--status should not request render escaping" || fail "$logged"
